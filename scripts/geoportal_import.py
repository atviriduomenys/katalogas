...
def main():
    ...
    start_index = 1
    res = requests.get("https://www.geoportal.lt/metadata-catalog/rest/find/document", params={
        "start": start_index,
        "f": "pjson"
    })
    try:
        records = json.loads(res.content)
        total_results = records.get('totalResults', 0)
    ...
    while records:
        for record in records:
            url = f"https://www.geoportal.lt/metadata-catalog/rest/document?id={record['id']}"
