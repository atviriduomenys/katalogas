...
def migrate_cms_files(pbar, root_path, folder):
    for resource in FileResource.objects.filter(
        ...
        if obj is None:
            print(f'{resource} does not have associated object, skipping...')
        elif not os.path.exists(file_path):
            print(f'File with path "{file_path}" was not found, skipping...')
        else:
            dj_file = DjangoFile(
                open(file_path, mode='rb'),
            ...
            if resource.type == IMAGE_TYPE:
                ...
                except (Image.DoesNotExist, SuspiciousFileOperation):
                    print(f"Error: Image.DoesNotExist: {resource.filename = }, {file_path = }")
                else:
                    obj.image = image
