...
class MockFacetForm:
    ...
    def __init__(self, selected_facets):
        self.selected_facets = selected_facets


@pytest.mark.django_db
def test_get_selected_value():
    facet_form = MockFacetForm(['key_exact:value'])
...
def test_get_selected_value_multiple():
    facet_form = MockFacetForm(['key_exact:value1', 'key_exact:value2'])
...
def test_get_selected_value_int():
    facet_form = MockFacetForm(['key_exact:1'])
...
def test_get_selected_value_multiple_int():
    facet_form = MockFacetForm(['key_exact:1', 'key_exact:2'])
...
def test_get_selected_value_int_value_error(rf: RequestFactory):
    facet_form = MockFacetForm(['key_exact:?'])
    selected_value = get_selected_value(facet_form, 'key')
