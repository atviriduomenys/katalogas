...
def create_dataservice(apps, schema_editor):
    ...
    if not DataServiceType.objects.filter(
        title="Creative Commons Attribution 4.0"
    ).exists():
...
def assign_datasets_to_dataservice(apps, schema_editor):
    ...
    dataset_distributions = DatasetDistribution.objects.filter(
        url_format__extension="UAPI"
    )
    dataset_ids = dataset_distributions.values_list("dataset_id", flat=True)
    datasets = Dataset.objects.filter(id__in=dataset_ids)
    dataservice = Dataset.objects.filter(
        service=True, endpoint_url="https://get.data.gov.lt"
    ).first()
    relation_type = Relation.objects.filter(name="service").first()

    for dataset in datasets:
        if not DatasetRelation.objects.filter(
            dataset=dataset, part_of=dataservice, relation=relation_type
        ).exists():
