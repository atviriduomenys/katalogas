...
class DatasetRepresentativeService:
    ...
    def _handle_organization_representative(self, representative: Representative, organization: Organization) -> None:
        ...
        if representative.role == Representative.COORDINATOR:
            raise RepresentativeCreationError("role", "Organizacijai gali būti suteikta tik tvarkytojo rolė")
    ...
    def _handle_new_representative(self, form: Form, representative: Representative) -> None:
        ...
        if not self._email_already_sent(representative.email):
            self._send_registration_email(form, representative)
            messages.info(self.request, "Naudotojui išsiųstas laiškas dėl registracijos")
    ...
    def _send_registration_email(self, form: Form, representative: Representative) -> None:
        serializer = URLSafeSerializer(settings.SECRET_KEY)
