...
class RequestIndex(SearchIndex, Indexable):
    ...
    dataset_status = MultiValueField(
        model_attr="dataset_statuses", faceted=True, default="UNASSIGNED"
    )
    organization = MultiValueField(
        model_attr="requestassignment_set__organization__pk", faceted=True, default=-1
    )
    ...
    parent_category = MultiValueField(
        model_attr="dataset_parent_categories", faceted=True, null=True
    )
