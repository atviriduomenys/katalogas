...
from vitrina.requests.forms import (
    RequestEditOrgForm,
    RequestForm,
...
class RequestPublicationStatsView(RequestStatsMixin, RequestListView):
    ...
    def get_context_data(self, **kwargs) -> dict:
        ...
        for label in labels:
            ...
            if indicator == "request-count":
                total += request_count
                item = {"display_value": label.year, "count": request_count}
                ...
                total += count
                item = {"display_value": label.year, "count": count}
                bar_chart_data.append(item)
    ...
    def form_valid(self, form):
        ...
        for org in orgs:
            ...
            requestA = RequestAssignment.objects.create(
                request=self.object, organization=org, status=self.object.status
            )
            requestA.save()
        self.object.save()
        Task.objects.create(
    ...
    def form_valid(self, form):
        ...
        if self.object.organizations.exists():
            ...
            org_subs = Subscription.objects.filter(
                Q(object_id__in=org_id_list) | Q(object_id=None),
                sub_type=Subscription.ORGANIZATION,
...
class RequestCreatePlanView(PermissionRequiredMixin, TemplateView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if form.is_valid():
            if form_type == "create_form":
                plan = form.save()
                PlanRequest.objects.create(plan=plan, request=self.request_obj)
