# Generated by Django 4.2.23 on 2025-11-10 10:53

from django.db import migrations, models
from django.db.models import Max

from vitrina.structure.models import VersionStatus


def populate_version_field_for_metadata(apps, schema_editor):
    Metadata = apps.get_model("vitrina_structure", "Metadata")
    Version = apps.get_model("vitrina_structure", "Version")

    datasets_used_by_metadata = Metadata.objects.values_list("dataset_id", flat=True).distinct()
    for dataset_id in datasets_used_by_metadata:
        draft_metadata_rows = Metadata.objects.filter(dataset_id=dataset_id, draft=1, metadata_version__isnull=True)
        max_version = Version.objects.filter(dataset_id=dataset_id).aggregate(
            max_ver=Max('version')
        )['max_ver']

        next_version = 1 if max_version is None else max_version + 1

        draft_version = Version.objects.get_or_create(
            version=next_version,
            dataset_id=dataset_id,
            status=VersionStatus.DRAFT
        )[0]
        draft_metadata_rows.update(metadata_version=draft_version)


class Migration(migrations.Migration):

    dependencies = [
        ('vitrina_structure', '0008_metadataversion_status'),
    ]

    operations = [
        migrations.AddField(
            model_name='version',
            name='external_version',
            field=models.CharField(blank=True, max_length=50, null=True, verbose_name='Versijos numeris'),
        ),
        migrations.AddField(
            model_name='version',
            name='major',
            field=models.IntegerField(blank=True, null=True, verbose_name='Pagrindinis versijos numeris'),
        ),
        migrations.AddField(
            model_name='version',
            name='minor',
            field=models.IntegerField(blank=True, null=True, verbose_name='Papildomas versijos numeris'),
        ),
        migrations.AddField(
            model_name='version',
            name='patch',
            field=models.IntegerField(blank=True, null=True, verbose_name='Pataisos versijos numeris'),
        ),
        migrations.AddField(
            model_name='version',
            name='status',
            field=models.CharField(choices=[('DRAFT', 'Juodraštis'), ('PRE_RELEASE', 'Išankstinis leidimas'), ('STABLE', 'Stabili'), ('DEPRECATED', 'Pasenusi'), ('WITHDRAWN', 'Atsisakyta'), ('DELETED', 'Ištrinta'), ('DESTROYED', 'Sunaikinta'), ('TESTING', 'Tikrinama'), ('DEPLOYING', 'Diegiama')], max_length=20, null=True, verbose_name='Versijos būsena'),
        ),
        migrations.AddField(
            model_name='version',
            name='version_type',
            field=models.CharField(blank=True, choices=[('MAJOR', 'Pagrindinė'), ('MINOR', 'Papildoma'), ('PATCH', 'Pataisa')], max_length=20, null=True, verbose_name='Versijos tipas'),
        ),
        migrations.RunPython(populate_version_field_for_metadata, migrations.RunPython.noop),
    ]
