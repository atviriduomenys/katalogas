...
DATETIME_DATA = {
    "_data": [
        {
            "_type": "datasets/gov/example/City/:summary/num",
            "bin": "2023-02-24T10:10:29.594309",
            "count": 100
        },
        {
            "_type": "datasets/gov/example/City/:summary/num",
            "_id": "dcffa0c0-421b-4d0b-ac6f-1b4a9df9b47c",
            "bin": "2023-02-24T09:10:29.594309",
            "count": 1
        },
        {
            "_type": "datasets/gov/example/City/:summary/num",
            "bin": "2023-02-23T10:10:29.594309",
            "count": 10
        },
    ]
}
...
def get_sorted_data_by_prop_type(
    data: dict,
    type: str = None,
    sort_key: str = 'bin'
):
    if type == 'integer' or type == 'boolean':
        sorted_data = sorted(data, key=lambda item: int(item[sort_key]))
    elif type == 'number':
        sorted_data = sorted(data, key=lambda item: float(item[sort_key]))
    else:
